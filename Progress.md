...
This is a sort of work diary that I include with the code to list progesses made. Consider the fact that sometimes I switch to a branch of the repository to try out some detail. If that fails I abandon the branch and as such, sometimes, the documentation of what has been attempted. Gaps can thus imply a step made to experiment with a detail that was not successful.
...
I need a global SpaceTransformation Object. This shoule be able to switch between normal and adjoint state, perform the optimization of shape and it should also contain all the shape information.
...
I found a core bug in the PML implementation which should improve the quality of the code substantially.  I am now running som precursor tests for the pml parameter study again which will lead into the large case examples. This might also solve my issues with the angled waveguide tests.
...
Now compiles again without warnings. Function handle for optimization also working. I have also reimplemented the computation of the material tensors in the inhomogenous rectangle translateion, which makes it easier to read and less prone to errors.
