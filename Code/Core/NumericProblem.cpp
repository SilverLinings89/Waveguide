...
std::complex<double> NumericProblem::evaluate_Energy_for_Position(double x,
                                                                  double y,
                                                                  double z) {
  dealii::Point<3, double> position(x, y, z);
  Vector<double> result(6);
...
void NumericProblem::Shift_Constraint_Matrix(
...
void NumericProblem::solve() {
  SolverControl lsc = SolverControl(30, 1.e-5, true, true);
...
Point<3, double> NumericProblem::transform_coordinate(
    const Point<3, double> in_p) {
...
NumericProblem::assemble_adjoint_local_contribution(double stepwidth) {
  ...
  for (unsigned int temp_counter = 0; temp_counter < 2; temp_counter++) {
    if (((GlobalParams.NumberProcesses % 2 == 1) &&
         (rank + 1 == GlobalParams.NumberProcesses / 2) && temp_counter == 0)) {
