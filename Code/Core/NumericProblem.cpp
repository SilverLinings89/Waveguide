...
void NumericProblem::make_constraints(
  ...
  // dealii::VectorTools::project(dof_handler, temp_cm, dealii::QGauss<3>(2), psf,
  //     exact_solution, false, dealii::QGauss<2>(2), false);
  ...
  for (unsigned int i = 0; i < restrained_dofs.size(); i++) {
    ...
    temp_cm.set_inhomogeneity(restrained_dofs[i] + shift,
        exact_solution[restrained_dofs[i]]);
  }
...
Tensor<1, 3, std::complex<double>> NumericProblem::Conjugate_Vector(
    dealii::Vector<std::complex<double>> input) {
  Tensor<1, 3, std::complex<double>> ret;
...
void NumericProblem::make_constraints(
    AffineConstraints<std::complex<double>> *in_constraints,
    unsigned int shift,
    unsigned int size) {
  dealii::Vector<std::complex<double>> exact_solution;
...
void NumericProblem::assemble_system(unsigned int shift,
    dealii::SparseMatrix<std::complex<double>> *matrix,
    dealii::Vector<std::complex<double>> *rhs,
    dealii::AffineConstraints<std::complex<double>> *in_constraints) {
  reinit_rhs();
  ...
  // const double eps_in = GlobalParams.M_W_epsilonin * e_temp;
  // const double eps_out = GlobalParams.M_W_epsilonout * e_temp;
  const double eps_in = 1.0;
  const double eps_out = 1.0;
  const double mu_zero = mu_temp;
  Vector<std::complex<double>> cell_rhs(dofs_per_cell);
  cell_rhs = 0;
  Tensor<2, 3, std::complex<double>> transformation;
    ...
    for (unsigned int q_index = 0; q_index < n_q_points; ++q_index) {
      ...
      for (unsigned int i = 0; i < dofs_per_cell; i++) {
        ...
        if (cell->point_inside(bounded_cell)) {
          dealii::Vector<std::complex<double>> JCurl;
          JCurl.reinit(3);
          dealii::Vector<std::complex<double>> JVal;
          JVal.reinit(3);
          psf.vector_curl(quadrature_points[q_index], JCurl);
          psf.vector_value(quadrature_points[q_index], JVal);
          cell_rhs[i] = (mu * I_Curl) * Conjugate_Vector(JCurl)
              * JxW
              - ((epsilon * I_Val) * Conjugate_Vector(JVal)) * JxW
                  * GlobalParams.C_omega * GlobalParams.C_omega;
        }
      }

      in_constraints->distribute_local_to_global(cell_matrix_real, cell_rhs,
          local_dof_indices, *matrix, *rhs, false);
    }
  }
