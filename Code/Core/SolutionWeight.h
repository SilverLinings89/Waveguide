 ...
 * Bei der Berechnung der L2-Norm sollen nur Werte innerhalb des Wellenleiters
 * betrachtet werden. Dies kann mit einer gewichtungs-Funktion eingestellt
 * werden, die bei integrate_difference mitübergeben wird. Diese Klasse dient
 * genau dazu 1 zurück zu geben, wenn die Koordinate im Wellenleiter und nicht
 * in der PML liegt und 0 sonst.
 *
 ...
 * \brief This function has internal usage to execute a function only on the
 * interior of the Waveguide.
 *
 * SolutionWeight is a class, that has been derived from the class Function and
 * which can be used to generate handles to functions, that return specific
 * values. The pattern of passing object built from classes derived from
 * Function is a commonly used one in Deal.II. This function offers a weight for
 * locations inside the waveguide. In order to integrate or calculate the
 * L2-norm inside the Waveguide, this is needed. The value of the function to be
 * used is multiplied with this weighing-function which returns 1 for any point
 * inside the Waveguide and 0 outside of it. Mathematically, this function can
 * be represented by \f[ f: \mathbb{R}^{dim} \to \{1,0\} , \, \boldsymbol{x}
 * \mapsto \begin{cases}1&\text{ for} \, \boldsymbol{x} \in \Omega_W \\ 0
