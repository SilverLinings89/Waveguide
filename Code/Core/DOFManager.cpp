...
void DOFManager::compute_and_send_x_dofs() {
  if (GlobalParams.Blocks_in_x_direction > 1 &&
      (GlobalParams.Index_in_x_direction <
    ...
    MPI_Send(bdof_vals, boundary_dofs.n_elements(), MPI_UNSIGNED,
             Geometry.get_neighbor_for_interface(Direction::PlusX).second, 0,
             MPI_COMM_WORLD);
  }
}
...
void DOFManager::compute_and_send_y_dofs() {
  if (GlobalParams.Blocks_in_y_direction > 1) {
    if (GlobalParams.Index_in_y_direction <
      ...
      MPI_Send(bdof_vals, boundary_dofs.n_elements(), MPI_UNSIGNED,
               Geometry.get_neighbor_for_interface(Direction::PlusY).second, 0,
               MPI_COMM_WORLD);
    }
  }
}
...
void DOFManager::compute_and_send_z_dofs() {
  if (GlobalParams.Blocks_in_z_direction > 1) {
    if (GlobalParams.Index_in_z_direction <
      ...
      if (GlobalParams.Index_in_z_direction !=
          GlobalParams.Coupling_Interface_Z_Block_Index - 1) {
        ...
        MPI_Send(bdof_vals, boundary_dofs.n_elements(), MPI_UNSIGNED,
                 Geometry.get_neighbor_for_interface(Direction::PlusZ).second,
                 0, MPI_COMM_WORLD);
      }
    }
  }
  ...
  if (GlobalParams.Blocks_in_x_direction > 1) {
    if (GlobalParams.Index_in_x_direction > 0) {
      ...
      MPI_Recv(bdof_vals, boundary_dofs.n_elements(), MPI_UNSIGNED,
               Geometry.get_neighbor_for_interface(Direction::MinusX).second, 0,
...
void DOFManager::receive_y_dofs() {
  if (GlobalParams.Blocks_in_y_direction > 1) {
    if (GlobalParams.Index_in_y_direction > 0) {
      ...
      int *bdof_vals = new int[boundary_dofs.n_elements()];
      MPI_Recv(bdof_vals, boundary_dofs.n_elements(), MPI_UNSIGNED,
               Geometry.get_neighbor_for_interface(Direction::MinusY).second, 0,
               MPI_COMM_WORLD, MPI_STATUS_IGNORE);
      IndexSet new_numbers;
      new_numbers.set_size(n_global_dofs);
      for (unsigned int i = 0; i < boundary_dofs.n_elements(); i++) {
...
void DOFManager::receive_z_dofs() {
  if (GlobalParams.Blocks_in_z_direction > 1) {
    if (GlobalParams.Index_in_z_direction > 0) {
      if (GlobalParams.Index_in_z_direction !=
          GlobalParams.Coupling_Interface_Z_Block_Index) {
        IndexSet boundary_dofs = get_dofs_for_boundary_id(4);
        int *bdof_vals = new int[boundary_dofs.n_elements()];
        MPI_Recv(bdof_vals, boundary_dofs.n_elements(), MPI_UNSIGNED,
                 Geometry.get_neighbor_for_interface(Direction::MinusZ).second,
...
void DOFManager::update_interface_dofs_with_IndexSet(
    IndexSet in_new_indices, types::boundary_id in_bid) {
  IndexSet local_numbering = get_dofs_for_boundary_id(in_bid);
  if (in_new_indices.n_elements() != local_numbering.n_elements()) {
    std::cout << "There was a communication problem in "
                 "update_interface_dofs_with_IndexSet."
              << std::endl;
  }
  ...
  for (unsigned int i = 0; i < all_local_dofs.size(); i++) {
    ...
    if (local_numbering.is_element(index)) {
      new_numbering.push_back(
          in_new_indices.nth_index_in_set(local_numbering.is_element(index)));
    } else {
...
void DOFManager::MPI_build_global_index_set_vector() {
  this->own_dofs_per_process =
      dealii::Utilities::MPI::create_ascending_partitioning(
          MPI_COMM_WORLD, this->n_locally_owned_dofs);
...
bool compareIndexCenterPairs(DofSortData c1, DofSortData c2) {
  ...
  std::cout << "There was an error in dof sorting - two object were "
               "indistinguishable."
            << std::endl;
  return false;
}
...
void DOFManager::SortDofs() {
  ...
  std::sort(dofs_sort_objects.begin(), dofs_sort_objects.end(),
            compareIndexCenterPairs);
  std::vector<unsigned int> new_numbering;
...
DOFManager::DOFManager(unsigned int i_dofs_per_cell,
                       unsigned int i_dofs_per_face,
