...
/**
 * \class Optimization
 * \brief This class is an abstract interface to describe the general workings
 * of an optimization scheme. It is used to compute optimization steps and
 * controls the DOFs of the ShapeTransformation.
 *
 * In general there are two kinds of Optimization Schemes derived from this
 * class. On the one hand there are finite-difference kind schemes which are
 * based on the idea of varying the value of one shape parameter slightly,
 * resolving the problem (which is now slightl varied compaired to the original
 * one) and hence computing the entry of the shape gradient. Repeating this
 * pattern for any un-restrained dof we can  compute the complete gradient. The
 * other version is based on an adjoint model where we solve the forward problem
 * and its dual and can compute the shape gradient for all DOFs from these two
 * results. \author Pascal Kraft \date 28.11.2016
 */
