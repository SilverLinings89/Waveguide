...
Point<3> InhomogenousTransformationCircular::math_to_phys(Point<3> coord) const {
  Point<3> ret;
...
Point<3> InhomogenousTransformationCircular::phys_to_math(Point<3> coord) const {
  Point<3> ret;
  if(coord[2] < GlobalParams.M_R_ZLength/(-2.0)) {
...
Tensor<2,3,std::complex<double>> InhomogenousTransformationCircular::get_Tensor(Point<3> & position) const {
  Tensor<2,3,double> transform = get_Space_Transformation_Tensor(position);
  return Apply_PML_To_Tensor(position, transform);
}

Tensor<2,3,std::complex<double>> InhomogenousTransformationCircular::get_Preconditioner_Tensor(Point<3> & position, int block) const {
  Tensor<2,3,double> transform = get_Space_Transformation_Tensor(position);
  return Apply_PML_To_Tensor_For_Preconditioner(position, transform, block);
}

Tensor<2,3,double> InhomogenousTransformationCircular::get_Space_Transformation_Tensor_Homogenized(Point<3> & position) const {
...
Tensor<2,3,double> InhomogenousTransformationCircular::get_Space_Transformation_Tensor(Point<3> & position) const {
  std::pair<int, double> sector_z = Z_to_Sector_and_local_z(position[2]);
...
Tensor<2,3, std::complex<double>> InhomogenousTransformationCircular::Apply_PML_To_Tensor(Point<3> & position, Tensor<2,3, double> transformation) const {
...
Tensor<2,3, std::complex<double>> InhomogenousTransformationCircular::Apply_PML_To_Tensor_For_Preconditioner(Point<3> & position, Tensor<2,3, double> transformation, int block) const {
  std::complex<double> S1(1.0, 0.0),S2(1.0,0.0), S3(1.0,0.0);
  Tensor<2,3, std::complex<double>> ret;
...
std::complex<double> InhomogenousTransformationCircular::gauss_product_2D_sphere(double z, int n, double R, double Xc, double Yc, Waveguide * in_w)
{
  double* r = NULL;
  double* t = NULL;
  double* q = NULL;
...
void InhomogenousTransformationCircular::set_free_dof(int in_dof, double in_val) {
  int dof = in_dof + 3;
  if(dof < (int)NDofs() -3 && dof >= 0) {
    int sector = floor(dof/3);
    if(sector == sectors) {
