#ifndef INHOMOGENOUS_TRANSFORMATION_CIRCULAR_CPP
...
InhomogenousTransformationCircular::InhomogenousTransformationCircular(
    int in_rank)
...
Point<3> InhomogenousTransformationCircular::math_to_phys(
    Point<3> coord) const {
  Point<3> ret;
  if (coord[2] < GlobalParams.M_R_ZLength / (-2.0)) {
    ...
    ret[1] = (2 * GlobalParams.M_C_Dim1In) * coord[1] /
             (GlobalParams.M_C_Dim1In + GlobalParams.M_C_Dim1Out);
    ret[2] = coord[2];
    ...
    ret[0] = (2 * GlobalParams.M_C_Dim1Out) * coord[0] /
             (GlobalParams.M_C_Dim1In + GlobalParams.M_C_Dim1Out);
    ret[1] = (2 * GlobalParams.M_C_Dim1Out) * coord[1] /
             (GlobalParams.M_C_Dim1In + GlobalParams.M_C_Dim1Out);
    ret[2] = coord[2];
  }
  return ret;
...
Point<3> InhomogenousTransformationCircular::phys_to_math(
    Point<3> coord) const {
  Point<3> ret;
  if (coord[2] < GlobalParams.M_R_ZLength / (-2.0)) {
    ret[0] = (GlobalParams.M_C_Dim1In + GlobalParams.M_C_Dim1Out) * coord[0] /
             (2 * GlobalParams.M_C_Dim1In);
    ret[1] = (GlobalParams.M_C_Dim1In + GlobalParams.M_C_Dim1Out) * coord[1] /
             (2 * GlobalParams.M_C_Dim1In);
    ret[2] = coord[2];
  } else if (coord[2] >= GlobalParams.M_R_ZLength / (-2.0) &&
             coord[2] < GlobalParams.M_R_ZLength / (2.0)) {
...
bool InhomogenousTransformationCircular::PML_in_X(Point<3> &p) const {
  return p(0) < XMinus || p(0) > XPlus;
}

bool InhomogenousTransformationCircular::PML_in_Y(Point<3> &p) const {
...
double InhomogenousTransformationCircular::Preconditioner_PML_Z_Distance(
...
double InhomogenousTransformationCircular::PML_Y_Distance(Point<3> &p) const {
  if (p(1) > 0) {
    return p(1) - YMinus;
  } else {
    return -p(1) - YPlus;
...
InhomogenousTransformationCircular::get_Tensor(Point<3> &position) const {
  Tensor<2, 3, double> transform = get_Space_Transformation_Tensor(position);
  return Apply_PML_To_Tensor(position, transform);
}

Tensor<2, 3, std::complex<double>>
InhomogenousTransformationCircular::get_Preconditioner_Tensor(
...
Tensor<2, 3, double>
InhomogenousTransformationCircular::get_Space_Transformation_Tensor_Homogenized(
  ...
  dist = sqrt(dist);
  double v1 = GlobalParams.M_R_XLength / 2.0 -
              std::min(GlobalParams.M_BC_XMinus, GlobalParams.M_BC_XPlus);
  double v2 = GlobalParams.M_R_YLength / 2.0 -
              std::min(GlobalParams.M_BC_YMinus, GlobalParams.M_BC_YPlus);
...
Tensor<2, 3, double>
InhomogenousTransformationCircular::get_Space_Transformation_Tensor(
    Point<3> &position) const {
  std::pair<int, double> sector_z = Z_to_Sector_and_local_z(position[2]);
...
Tensor<2, 3, std::complex<double>>
InhomogenousTransformationCircular::Apply_PML_To_Tensor(
  ...
  // * ((System_Coordinate_in_Waveguide(position))?GlobalParams.M_W_epsilonin :
  // GlobalParams.M_W_epsilonout);
  ...
  if (PML_in_X(position)) {
    ...
    sigmax = pow(r / d, GlobalParams.M_BC_DampeningExponent) *
  ...
  if (PML_in_Y(position)) {
    ...
    sigmay = pow(r / d, GlobalParams.M_BC_DampeningExponent) *
  ...
  if (PML_in_Z(position)) {
    ...
    sigmaz = pow(r / d, GlobalParams.M_BC_DampeningExponent) *
...
Tensor<2, 3, std::complex<double>>
InhomogenousTransformationCircular::Apply_PML_To_Tensor_For_Preconditioner(
  ...
  if (Preconditioner_PML_in_Z(position, block)) {
    ...
    sigmaz = pow(r / d, GlobalParams.M_BC_DampeningExponent) *
             GlobalParams.M_BC_SigmaZMax;
    sz_p.real(1 + pow(r / d, GlobalParams.M_BC_DampeningExponent) *
...
std::complex<double>
InhomogenousTransformationCircular::gauss_product_2D_sphere(double z, int n,
                                                            double R, double Xc,
                                                            double Yc,
                                                            Waveguide *in_w) {
  double *r = NULL;
...
double InhomogenousTransformationCircular::get_dof(int dof) const {
  if (dof < (int)NDofs() && dof >= 0) {
    ...
    std::cout << "Critical: DOF-index out of bounds in "
                 "HomogenousTransformationCircular::get_dof!"
              << std::endl;
    return 0.0;
  }
}

double InhomogenousTransformationCircular::get_free_dof(int in_dof) const {
  int dof = in_dof + 3;
  if (dof < (int)NDofs() - 3 && dof >= 0) {
    ...
    std::cout << "Critical: DOF-index out of bounds in "
                 "HomogenousTransformationCircular::get_free_dof!"
              << std::endl;
    return 0.0;
  }
}

void InhomogenousTransformationCircular::set_dof(int dof, double in_val) {
  if (dof < (int)NDofs() && dof >= 0) {
    int sector = floor(dof / 3);
    if (sector == sectors) {
    ...
    std::cout << "Critical: DOF-index out of bounds in "
                 "HomogenousTransformationCircular::set_dof!"
              << std::endl;
  }
}

void InhomogenousTransformationCircular::set_free_dof(int in_dof,
                                                      double in_val) {
  int dof = in_dof + 3;
  if (dof < (int)NDofs() - 3 && dof >= 0) {
    int sector = floor(dof / 3);
    ...
    std::cout << "Critical: DOF-index out of bounds in "
                 "HomogenousTransformationCircular::set_free_dof!"
              << std::endl;
  }
}

double InhomogenousTransformationCircular::Sector_Length() const {
  return GlobalParams.M_R_ZLength / (double)GlobalParams.M_W_Sectors;
}

void InhomogenousTransformationCircular::estimate_and_initialize() {
  ...
  if (sectors == 1) {
    Sector<3> temp12(true, true, -GlobalParams.M_R_ZLength / 2,
                     GlobalParams.M_R_ZLength / 2);
    ...
    Sector<3> temp(true, false, -GlobalParams.M_R_ZLength / (2.0),
                   -GlobalParams.M_R_ZLength / 2.0 + length);
    case_sectors.push_back(temp);
    for (int i = 1; i < sectors; i++) {
      Sector<3> temp2(false, false,
    ...
    case_sectors[0].set_properties_force(
        m_0, InterpolationPolynomialZeroDerivative(length_rel, m_0, m_1), r_0,
        InterpolationPolynomialZeroDerivative(length_rel, r_0, r_1), 0,
...
double InhomogenousTransformationCircular::get_r(double z_in) const {
  std::pair<int, double> two = Z_to_Sector_and_local_z(z_in);
  return case_sectors[two.first].get_r(two.second);
}

double InhomogenousTransformationCircular::get_m(double z_in) const {
  std::pair<int, double> two = Z_to_Sector_and_local_z(z_in);
  return case_sectors[two.first].get_m(two.second);
}

double InhomogenousTransformationCircular::get_v(double z_in) const {
  std::pair<int, double> two = Z_to_Sector_and_local_z(z_in);
  return case_sectors[two.first].get_v(two.second);
}

double InhomogenousTransformationCircular::get_Q1(double z_in) const {
  std::pair<int, double> two = Z_to_Sector_and_local_z(z_in);
  return case_sectors[two.first].getQ1(two.second);
}

double InhomogenousTransformationCircular::get_Q2(double z_in) const {
  std::pair<int, double> two = Z_to_Sector_and_local_z(z_in);
  return case_sectors[two.first].getQ2(two.second);
}

double InhomogenousTransformationCircular::get_Q3(double z_in) const {
  std::pair<int, double> two = Z_to_Sector_and_local_z(z_in);
  return case_sectors[two.first].getQ3(two.second);
}

Vector<double> InhomogenousTransformationCircular::Dofs() const {
  Vector<double> ret;
...
unsigned int InhomogenousTransformationCircular::NFreeDofs() const {
  return NDofs() - 6;
}

bool InhomogenousTransformationCircular::IsDofFree(int index) const {
  return index > 2 && index < (int)NDofs() - 3;
}

void InhomogenousTransformationCircular::Print() const {
  std::cout << "Printing is not yet implemented." << std::endl;
}

unsigned int InhomogenousTransformationCircular::NDofs() const {
  return sectors * 3 + 3;
}
#endif
