...
Point<3> HomogenousTransformationCircular::math_to_phys_hom(
    Point<3> coord) const {
  Point<3> ret;
  double alpha = 0.0;
  ...
  double v1 = GlobalParams.M_R_XLength / 2.0 -
              std::max(GlobalParams.M_BC_XMinus, GlobalParams.M_BC_XPlus);
  double v2 = GlobalParams.M_R_YLength / 2.0 -
              std::max(GlobalParams.M_BC_YMinus, GlobalParams.M_BC_YPlus);
...
Point<3> HomogenousTransformationCircular::phys_to_math_hom(
    Point<3> coord) const {
  Point<3> ret;
  if (coord[2] < GlobalParams.M_R_ZLength / (-2.0)) {
    ret[0] = (GlobalParams.M_C_Dim1In + GlobalParams.M_C_Dim1Out) * coord[0] /
...
Tensor<2, 3, std::complex<double>> HomogenousTransformationCircular::get_Tensor(
    Point<3> &position) const {
  Tensor<2, 3, double> transform =
      get_Space_Transformation_Tensor_Homogenized(position);
  return Apply_PML_To_Tensor(position, transform);
}

Tensor<2, 3, std::complex<double>>
HomogenousTransformationCircular::get_Preconditioner_Tensor(Point<3> &position,
                                                            int block) const {
  Tensor<2, 3, double> transform =
      get_Space_Transformation_Tensor_Homogenized(position);
...
Tensor<2, 3, double>
HomogenousTransformationCircular::get_Space_Transformation_Tensor_Homogenized(
    Point<3> &position) const {
...
Tensor<2, 3, double>
HomogenousTransformationCircular::get_Space_Transformation_Tensor(
    Point<3> &position) const {
  std::pair<int, double> sector_z = Z_to_Sector_and_local_z(position[2]);
...
Tensor<2, 3, std::complex<double>>
HomogenousTransformationCircular::Apply_PML_To_Tensor(
...
Tensor<2, 3, std::complex<double>>
HomogenousTransformationCircular::Apply_PML_To_Tensor_For_Preconditioner(
    Point<3> &position, Tensor<2, 3, double> transformation, int rank) const {
  Tensor<2, 3, std::complex<double>> MaterialTensor;
  ...
  sz_p.real(pow(r_temp / d_temp, GlobalParams.M_BC_DampeningExponent) *
            GlobalParams.M_BC_KappaZMax);
  sz_p.imag(pow(r_temp / d_temp, GlobalParams.M_BC_DampeningExponent) *
            GlobalParams.M_BC_SigmaZMax);
...
std::complex<double>
HomogenousTransformationCircular::gauss_product_2D_sphere_primal(
