...
RectangularMode::RectangularMode(double eps_in, double eps_out, double x_width_waveguide, double y_width_waveguide, double x_width_domain, double y_width_domain, double beta, unsigned int order)
    :FEM_order(order),
...
void RectangularMode::make_mesh() {
    Point<3> p1 = new Point<3>(-x_width_domain / 2.0, -y_width_domain / 2.0, -0.1);
    Point<3> p2 = new Point<3>( x_width_domain / 2.0,  y_width_domain / 2.0, 0.1);
    std::vector<unsigned int> repetitions;
    repetitions.push_back(100);
    repetitions.push_back(100);
    repetitions.push_back(0);
    GridGenerator::fubdivided_hyper_rectangle(triangulation, repetitions, p1, p2, true);
...
void RectangularMode::make_boundary_conditions(){
    ...
    VectorTools::project_boundary_values_curl_conforming_l2(dof_handler, 0, zero, 0, constraints);
    VectorTools::project_boundary_values_curl_conforming_l2(dof_handler, 0, zero, 1, constraints);
    VectorTools::project_boundary_values_curl_conforming_l2(dof_handler, 0, zero, 2, constraints);
    VectorTools::project_boundary_values_curl_conforming_l2(dof_handler, 0, zero, 3, constraints);
    constraints.close();
};
