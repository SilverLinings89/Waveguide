...
double LaguerreFunction::evaluate(unsigned int n, unsigned int m, double x) {
    ...
    for(unsigned int k = 0; k <= n; k++) {
        ret += (double)binomial_coefficient(n+m, n-k) * (std::pow(-x,k) / (double)factorial(k));
    }
    return ret;
}
...
unsigned int LaguerreFunction::binomial_coefficient(unsigned int n, unsigned int k) {
    if( k > n) return 0;
