...
void NeighborSurface::send() {
	...
	int return_value = MPI_Send(&send_buffer, n_dofs, MPI_UNSIGNED, local_partner_mpi_rank, tag, GlobalMPI.communicators_by_level[level]);
	if(return_value != 0) {
...
void NeighborSurface::receive() {
	...
	int status = MPI_Recv(&recv_buffer, n_dofs, MPI_UNSIGNED, local_partner_mpi_rank, tag, GlobalMPI.communicators_by_level[level], &recv_status);
	if(status != 0) {
