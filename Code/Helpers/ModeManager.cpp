/*
 * ModeManager.cpp
 * \brief this class is supposed to handle general modal computations. It can compute the analytic shape of a mode for circular connectors as well as loading data from an input file for rectangular connectors.
 * \date 23.03.2017
 * \author Pascal Kraft
 */
...
double ModeManager::get_input_component(int in_component, dealii::Point<3, double> position, int mode){
	if(!in_prepared) {
		prepare_mode_in();
	}
	if(in_circular){
...
double ModeManager::get_output_component(int in_component, dealii::Point<3, double> position, int mode){
	if(!out_prepared) {
		prepare_mode_out();
	}
	if(out_circular){
...
double ModeManager::bessi0 (double x) {
    ...
    if(abs(x) < 3.75) {
        ...
        double ax = std::abs(x);
        double y = 3.75/ax;
        return (std::exp(ax)/std::sqrt(ax))*(q1+y*(q2+y*(q3+y*(q4+y*(q5+y*(q6+y*(q7+y*(q8+y*q9))))))));
    }
}
...
double ModeManager::bessi1 (double x) {
    ...
    if(std::abs(x) < 3.75) {
        ...
        double ax = std::abs(x);
        double y = 3.75/ax;
        double ret = (std::exp(ax)/std::sqrt(ax))*(q1+y*(q2+y*(q3+y*(q4+y*(q5+y*(q6+y*(q7+y*(q8+y*q9))))))));
        if (x<0) ret *= -1.0;
        return ret;
    }
}
...
double ModeManager::J_0( double x) {
    ...
    if(std::abs(x) < 8) {
        ...
        return std::sqrt(.636619772/ax)*(std::cos(xx)*(p1+y*(p2+y*(p3+y*(p4+y *p5))))-z*std::sin(xx)*(q1+y*(q2+y*(q3+y*(q4+y*q5)))));
    }
}
...
double ModeManager::J_1( double x) {
    ...
    if(std::abs(x) < 8 ) {
        ...
        return std::sqrt(.636619772/ax)*(std::cos(xx)*(p1+y*(p2+y*(p3+y*(p4+y*p5))))-z*std::sin(xx)*(q1+y*(q2+y*(q3+y*(q4+y*q5)))))*std::copysign(1.,x);
    }
}
...
double ModeManager::K_0( double x) {
    ...
    if( x < 2.0) {
        double y = x*x/4.0;
        return (-log(x/2.0)*bessi0(x))+(p1+y*(p2+y*(p3+y*(p4+y*(p5+y*(p6+y*p7))))));
    } else {
        double y = 2.0/x;
        return (exp(-x)/sqrt(x))*(q1+y*(q2+y*(q3+y*(q4+y*(q5+y*(q6+y*q7))))));
    }
}
...
double ModeManager::K_1( double x) {
    ...
    if(x < 2.0) {
        double y = x*x/4.0;
        return (log(x/2.0)*bessi1(x))+(1.0/x)*(p1+y*(p2+y*(p3+y*(p4+y*(p5+y*(p6+y*p7))))));
    } else {
        double y = 2.0 / x;
        return (exp(-x)/sqrt(x))*(q1+y*(q2+y*(q3+y*(q4+y*(q5+y*(q6+y*q7))))));
    }
...
double ModeManager::get_lhs(double u) {
    return -u * J_0(u) / J_1(u);
}
