...
RoundMeshGenerator::RoundMeshGenerator(SpaceTransformation *in_ct)
    : MaxDistX((GlobalParams.M_C_Dim1Out + GlobalParams.M_C_Dim1In) * 1.4 /
               2.0),
      MaxDistY((GlobalParams.M_C_Dim1Out + GlobalParams.M_C_Dim1In) * 1.4 /
               2.0) {
...
void RoundMeshGenerator::prepare_triangulation(
  ...
  for (int i = 0; i < GlobalParams.R_Local; i++) {
    ...
    MaxDistFromBoundary =
        (MaxDistFromBoundary +
         ((GlobalParams.M_C_Dim1Out + GlobalParams.M_C_Dim1In) / 2.0)) /
        2.0;
  }
  ...
  for (int i = 0; i < GlobalParams.R_Interior; i++) {
    cell = in_tria->begin_active();
    for (; cell != endc; ++cell) {
      if (Distance2D(cell->center(true, false)) <
          (GlobalParams.M_C_Dim1In + GlobalParams.M_C_Dim1Out) / 2.0) {
        cell->set_refine_flag();
      }
    }
  ...
  /// mesh_info(*in_tria,
  /// "Output"+std::to_string(GlobalParams.MPI_Rank)+".vtk");
