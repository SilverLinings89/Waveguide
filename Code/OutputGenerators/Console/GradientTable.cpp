...
GradientTable::GradientTable(unsigned int in_step,
                             dealii::Vector<double> in_configuration,
                             double in_quality,
                             dealii::Vector<double> in_last_configuration,
                             double in_last_quality)
        : ndofs(100), nfreedofs(100), GlobalStep(in_step) {
    final_quality = 0.0;
...
void GradientTable::AddComputationResult(int in_component, double in_step,
                                         double in_quality) {
    steps[in_component] = in_step;
    qualities[in_component] = in_quality;
}

void GradientTable::AddFullStepResult(dealii::Vector<double> in_step,
                                      double in_quality) {
    for (int i = 0; i < nfreedofs; i++) {
