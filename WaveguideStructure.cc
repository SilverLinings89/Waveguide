...
Tensor<2,3, double> Sector::TransformationTensorInternal (double in_x, double in_y, double z) {
	...
	double help2 = (help1*(v_0 - 6*m_0*z + 6*m_1*z - 4*v_0*z - 2*v_1*z + 6*m_0*zz - 6*m_1*zz + 3*v_0*zz + 3*v_1*zz) + 6*z*(r_0 - r_1)*(z - 1)*(m_0 + zzz*(2*m_0 - 2*m_1 + v_0 + v_1) + v_0*z - zz*(3*m_0 - 3*m_1 + 2*v_0 + v_1)))/help1 - (6*y*z*(r_0 - r_1)*(z - 1))/help1;
	Tensor<2,3,double> u;
	u[0][0] = temp;
	u[0][1] = 0.0;
	u[0][2] = 0.0;
